...
{% macro normalize_priority(col) %}
    case
        when cast({{ col }} as varchar) in ('1', '2', '3', '4') then cast({{ col }} as integer)
        when lower(cast({{ col }} as varchar)) = 'urgent' then 1
        when lower(cast({{ col }} as varchar)) = 'high' then 2
        when lower(cast({{ col }} as varchar)) = 'medium' then 3
        when lower(cast({{ col }} as varchar)) = 'low' then 4
        else null
    end
{% endmacro %}
...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or cast({{ col }} as varchar) = '' then 'priority bị trống'
        when try_cast({{ col }} as integer) is not null then 'priority là số ngoài khoảng 1..4'
        else 'priority là chuỗi lạ không quy đổi được'
    end
{% endmacro %}
